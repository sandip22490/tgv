...
exports.use = async (tfVer) => {
  try {
    ...
    console.log(`${P_INFO}Switching to terragrunt ${version}${P_END}`);

    const source = join(__dirname, '../../store', version);

    if (!existsSync(source)) {
      console.log(`${P_ERROR}terragrunt ${version} is not installed${P_END}`);
      return console.log(`To install this version Run: ${P_OK}tgv install ${version}${P_END}`);
    }
    ...
    if (os.platform() === 'win32') {
      mkdirSync('C:\\terragrunt', { recursive: true });
      destination = 'C:\\terragrunt\\terragrunt.exe';
